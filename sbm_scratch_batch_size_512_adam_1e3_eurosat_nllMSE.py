...
def sbm_finetune(source_loader, target_loader, target_name , num_epochs, ): 
    ...
    ###############################################################################################
    # load resnet18 model
    save_dir = './logs/sbm_scratch_adam_1e3_nllMSE/eurosat/'    
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(base_loader):
            ...
            target_batch_2, _ = next(iter(target_loader))
            target_batch_2 = Variable(target_batch_2.cuda())

            model.eval()

            target_output = model(target_batch)
            target_output_2 = model(target_batch_2)
            ...
            ###############################################
            log_probability_base = F.log_softmax(source_outputs, dim=1)


            ce_loss = loss_CE(source_outputs, source_labels)
            mse_loss = loss_MSE(shifted_scores, source_outputs)
            nll_loss = loss_NLL(log_probability_base, source_labels)

            loss = ce_loss + mse_loss + nll_loss
        ...
        if (epoch % 25==0):
            ...
            torch.save(model.state_dict(), save_dir + '{}_epoch{}_sbm_scratch_NLL_batch_size_128.pth'.format(target_name, epoch))
